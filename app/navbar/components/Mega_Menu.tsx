        ...
        {data.columns.map((col, i) => (
          <div key={col.title} className="flex">
            <div className="flex-1 min-w-[200px]">
              <p className="text-[9px] tracking-[2px] uppercase text-red-700 font-semibold mb-4 font-raleway">
                {col.title}
              </p>
              ...
              {col.links.map((link) => (
                <a
                  ...
                  href={link.href}
                  className={`
                    block text-[12px] mb-2.5 tracking-[0.3px] font-raleway
                    transition-colors duration-150
                    ${link.isViewAll
                      ? 'text-red-700 underline text-[11px] mt-2'
              ...
              {col.showImage && (
                <div className="mt-5 w-40 h-44 bg-gradient-to-br from-pink-200 to-red-200 rounded flex items-end justify-center pb-3">
                  <span className="text-[9px] tracking-[2px] uppercase text-red-800 font-raleway">
                    Collection
                  </span>
                </div>
              )}
